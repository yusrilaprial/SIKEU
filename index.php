...
while ($datahariankeluar = mysqli_fetch_array($pengeluaran_hari_ini)){
  $hariankeluar[] = $datahariankeluar['jumlah'];

}
if($hariankeluar>0){
$jumlahhariankeluar = array_sum($hariankeluar);
}else{
$jumlahhariankeluar = 0;
}

$pemasukan_hari_ini = mysqli_query($koneksi, "SELECT jumlah FROM pemasukan where tgl_pemasukan = CURDATE()");
while ($dataharianmasuk = mysqli_fetch_array($pemasukan_hari_ini)){
  $harianmasuk[] = $dataharianmasuk['jumlah'];
}
if($harianmasuk>0){
  $jumlahharianmasuk = array_sum($harianmasuk);
}else{
$jumlahharianmasuk = 0;
}
...
$pendapatan = mysqli_num_rows($pemasukan);
...
*/
$satuhari1 =mysqli_query($koneksi, "SELECT jumlah FROM pemasukan
WHERE tgl_pemasukan = CURDATE() - INTERVAL 1 DAY");
while ($satuhari2= mysqli_fetch_array($satuhari1)){
  $satuhari3[] = $satuhari2['jumlah'];
}
if($satuhari3>0){
  $satuhari = array_sum($satuhari3);
}else{
$satuhari = NULL;
}


$duahari1 =mysqli_query($koneksi, "SELECT jumlah FROM pemasukan
WHERE tgl_pemasukan = CURDATE() - INTERVAL 2 DAY");
while ($duahari2= mysqli_fetch_array($duahari1)){
  $duahari3[] = $duahari2['jumlah'];
}
if($duahari3>0){
  $duahari = array_sum($duahari3);
}else{
$duahari = NULL;
}

$tigahari1 =mysqli_query($koneksi, "SELECT jumlah FROM pemasukan
WHERE tgl_pemasukan = CURDATE() - INTERVAL 3 DAY");
while ($tigahari2= mysqli_fetch_array($tigahari1)){
  $tigahari3[] = $tigahari2['jumlah'];
}
if($tigahari3>0){
  $tigahari = array_sum($tigahari3);
}else{
$tigahari = NULL;
}

$empathari1 =mysqli_query($koneksi, "SELECT jumlah FROM pemasukan
WHERE tgl_pemasukan = CURDATE() - INTERVAL 4 DAY");
while ($empathari2= mysqli_fetch_array($empathari1)){
  $empathari3[] = $empathari2['jumlah'];
}
if($empathari3>0){
  $empathari = array_sum($empathari3);
}else{
$empathari = NULL;
}

$limahari1 =mysqli_query($koneksi, "SELECT jumlah FROM pemasukan
WHERE tgl_pemasukan = CURDATE() - INTERVAL 5 DAY");
while ($limahari2= mysqli_fetch_array($limahari1)){
  $limahari3[] = $limahari2['jumlah'];
}
if($limahari3>0){
  $limahari = array_sum($limahari3);
}else{
$limahari = NULL;
}

$enamhari1 =mysqli_query($koneksi, "SELECT jumlah FROM pemasukan
WHERE tgl_pemasukan = CURDATE() - INTERVAL 6 DAY");
while ($enamhari2= mysqli_fetch_array($enamhari1)){
  $enamhari3[] = $enamhari2['jumlah'];
}
if($enamhari3>0){
  $enamhari = array_sum($enamhari3);
}else{
$enamhari = NULL;
}

$tujuhhari1 =mysqli_query($koneksi, "SELECT jumlah FROM pemasukan
WHERE tgl_pemasukan = CURDATE() - INTERVAL 7 DAY");
while ($tujuhhari2= mysqli_fetch_array($tujuhhari1)){
  $tujuhhari3[] = $tujuhhari2['jumlah'];
}
if($tujuhhari3>0){
  $tujuhhari = array_sum($tujuhhari3);
}else{
$tujuhhari = NULL;
}
?>
...
<?php require 'user.php'; ?>
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <a href="export-semua.php" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm"><i class="fas fa-download fa-sm text-white-50"></i> Download Laporan</a>
          </div>
                ...
                <div class="card-body">
                  <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                      ...
                      <div class="h5 mb-0 font-weight-bold text-gray-800">Rp.<?= number_format($jumlahharianmasuk,2,',','.');?></div>
                    </div>
                    <div class="col-auto">
                ...
                <div class="card-body">
                  <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                      ...
                      <div class="h5 mb-0 font-weight-bold text-gray-800">Rp.<?= number_format($jumlahhariankeluar,2,',','.');?></div>
                    </div>
                    <div class="col-auto">
                    ...
                    <div class="col-auto">
                      <i class="fas fa-clipboard-list fa-2x text-gray-300"></i>
						  ...
						  ?>
                            ...
                            <div class="progress-bar bg-<?=$warna?>" role="progressbar" style="width: 100%" aria-valuenow="<?=$value?>" aria-valuemin="0" aria-valuemax="100"><span><?=floor($value)?> % </span></div>
                          </div>
                        </div>
              </div>
            </div>
...
var myPieChart = new Chart(ctx, {
  ...
  data: {
    ...
    datasets: [{
      data: [<?php echo $jumlahmasuk ?>, <?php echo $jumlahkeluar ?>, <?php echo $uang ?>],
